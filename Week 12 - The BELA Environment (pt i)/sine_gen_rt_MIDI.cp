...
{
	gFrequency = powf(2.0, (noteNumber - 69)/12.0) * 440; //Frequency from MIDI note number
	float db = map(velocity, 1, 127, -40, 0); //Map velocity to db scale
	gAmplitude = powf(10, db/20.0);
	gAmplitude = 0.5;
...
{
	gIsNoteOn = false; //note-off is equal to note-on with velocity 0...
}
